...
import { formatAvatarInitials, formatPathname, formatUsername } from "@/utils/utilities"
  ...
  return (
    <div className="h-screen overflow-hidden flex items-center justify-center">
      ...
      <div className="h-full w-full bg-gray-50 relative overflow-y-auto lg:ml-64">
        <nav className="bg-white border-b border-gray-200 fixed z-30 w-full px-3 py-2 lg:px-5 lg:pl-3">
          <div className="flex bg-white w-3/4 items-center justify-between">
            ...
            <div className="flex flex-row gap-x-3 bg-gray-100 px-4 py-2 rounded-xl place-items-center justify-center">
              <div className="flex flex-1 flex-col justify-end">
                <h1 className="text-sm font-medium text-end">{formatUsername(authState.firstName,authState.lastName)}</h1>
                <p className="text-[12px] font-normal text-gray-400">{authState.email}</p>
              </div>
              <div>
                <Avatar className="bg-green-600 text-white">{formatAvatarInitials(authState.firstName,authState.lastName)}</Avatar>
              </div>
            </div>
          </div>
        </nav>
        <div className="pt-12 mt-12 px-8">
          {children}
        </div>
      </div>
    </div >
