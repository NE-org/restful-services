...
const LoginForm = () => {
    ...
    */
    ...
    const handleOnSubmit = () => {
        form.validateFields()
            // Enable login function after field validation
            .then(async (values) => {
                await login(values);
            })
            .catch(info => {
    ...
    return (
        <div className="border border-gray-300 rounded-md p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-md:mx-auto">
            <Form name='normal_login' onFinish={handleOnSubmit} layout='vertical' requiredMark className="space-y-6" form={form}>
                ...
                <Text className="text-sm !mt-10 text-center">Don't have an account <Link href="/register" className="text-green-600 hover:underline ml-1 whitespace-nowrap">Register here</Link></Text>
            </Form>
        </div>
    )
}
